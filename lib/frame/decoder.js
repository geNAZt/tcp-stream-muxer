...
/*
 ...
 type=Type of the Buffered Object (0=raw,1=json,2=new stream,3=binary end)          8bit int        6
...
Decoder.prototype.parsePackage = function (data) {
    ...
    if (this.state === -1) {
        //New Package arrives
        this.length = data.readInt32BE(0); //How long should it be ?
        this.streamID = data.readInt8(4); //To which stream should the data be written ?
        this.state = data.readInt8(5); //What Package is this ?
        ...
        //Is the package bigger then the data i need ?
        if (data.length > (this.length - this.lengthParsed)) {
            this.state = -1; //Set state to new package
    ...
    if (this.state === 1 || this.state === 2) {
        ...
        if (this.length === this.lengthParsed) {
            ...
            if (this.state === 2) {
                ...
                this.streamPool.emitToStream(this.streamID, obj);
            }

            this.state = -1;
        }
    } else if (this.state === 0) {
        this.streamPool.emitToStream(this.streamID, data);
